...
function MapMarker({
  ...
  const marker = useMemo(() => {
    ...
    const m = new MapLibreGL.Marker({ ...markerOptions, element: el, draggable })
    ...
    m.on("dragstart", () => { const ll = m.getLngLat(); callbacksRef.current.onDragStart?.({ lng: ll.lng, lat: ll.lat }); });
    m.on("drag", () => { const ll = m.getLngLat(); callbacksRef.current.onDrag?.({ lng: ll.lng, lat: ll.lat }); });
    m.on("dragend", () => { const ll = m.getLngLat(); callbacksRef.current.onDragEnd?.({ lng: ll.lng, lat: ll.lat }); });
    return m;
  }, []);
  ...
  const curOff = marker.getOffset();
  const newOff = markerOptions.offset ?? [0, 0];
  const [nx, ny] = Array.isArray(newOff) ? newOff : [newOff.x, newOff.y];
  if (curOff.x !== nx || curOff.y !== ny) marker.setOffset(newOff);
...
function MarkerPopup({ children, className, closeButton = false, ...popupOptions }: MarkerPopupProps) {
  ...
  const prevRef = useRef(popupOptions);
  const popup = useMemo(() => {
    const p = new MapLibreGL.Popup({ offset: 16, ...popupOptions, closeButton: false })
...
function MarkerLabel({ children, className, position = "top" }: MarkerLabelProps) {
  const cls = { top: "bottom-full mb-1", bottom: "top-full mt-1" };
  return (
    <div className={cn("absolute left-1/2 -translate-x-1/2 whitespace-nowrap text-foreground text-[10px] font-medium", cls[position], className)}>
      {children}
    </div>
  );
}
...
function MapRoute({
  ...
  useEffect(() => {
    ...
    const hClick = () => { onClick?.(); };
    const hEnter = () => { map.getCanvas().style.cursor = "pointer"; onMouseEnter?.(); };
    const hLeave = () => { map.getCanvas().style.cursor = ""; onMouseLeave?.(); };
    map.on("click", layerId, hClick);
    map.on("mouseenter", layerId, hEnter);
    map.on("mouseleave", layerId, hLeave);
    return () => { map.off("click", layerId, hClick); map.off("mouseenter", layerId, hEnter); map.off("mouseleave", layerId, hLeave); };
...
function MapArc<T extends MapArcDatum = MapArcDatum>({
  data, id: propId, curvature = DEFAULT_ARC_CURVATURE, samples = DEFAULT_ARC_SAMPLES,
  ...
  useEffect(() => {
    ...
    return () => {
      try { if (map.getLayer(layerId)) map.removeLayer(layerId); if (map.getLayer(hitLayerId)) map.removeLayer(hitLayerId); if (map.getSource(sourceId)) map.removeSource(sourceId); } catch {}
    };
  }, [isLoaded, map]);
...
function MapClusterLayer<P extends GeoJSON.GeoJsonProperties = GeoJSON.GeoJsonProperties>({
  ...
  useEffect(() => {
    ...
    const hEnterCluster = () => { map.getCanvas().style.cursor = "pointer"; };
    const hLeaveCluster = () => { map.getCanvas().style.cursor = ""; };
    const hEnterPoint = () => { if (onPointClick) map.getCanvas().style.cursor = "pointer"; };
    const hLeavePoint = () => { map.getCanvas().style.cursor = ""; };
    map.on("click", clusterLayerId, handleClusterClick);
    map.on("click", unclusteredLayerId, handlePointClick);
    map.on("mouseenter", clusterLayerId, hEnterCluster);
    map.on("mouseleave", clusterLayerId, hLeaveCluster);
    map.on("mouseenter", unclusteredLayerId, hEnterPoint);
    map.on("mouseleave", unclusteredLayerId, hLeavePoint);
    return () => {
      map.off("click", clusterLayerId, handleClusterClick);
      map.off("click", unclusteredLayerId, handlePointClick);
      map.off("mouseenter", clusterLayerId, hEnterCluster);
      map.off("mouseleave", clusterLayerId, hLeaveCluster);
      map.off("mouseenter", unclusteredLayerId, hEnterPoint);
      map.off("mouseleave", unclusteredLayerId, hLeavePoint);
    };
