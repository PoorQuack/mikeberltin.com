...
const useThemeSync = () => {
  ...
  useEffect(() => {
    ...
    const onStorage = (event: StorageEvent) => { if (event.key === "hero-theme" || event.key === "bento-theme") sync(); };
...
function HeroOrbitDeck() {
  ...
  const toggleTheme = () => {
    ...
    if (typeof window !== "undefined") {
      try { window.localStorage?.setItem("hero-theme", next); } catch {}
    }
    setTheme(next);
  };
  ...
  const modes = useMemo(() => ({
    strategy: {
      ...
      description: "Frame the launch protocol with clarity, align stakeholders in monochrome calm, and freeze decisive signals in under two screens.",
      items: ["Narrative arcs mapped to delivery windows", "Artifacts pre-styled in b/w for speed", "Decision rails surfaced inline"],
    },
    execution: {
      title: "Execution loop",
      description: "Deploy orchestration cues, sync distributed crews, and keep response telemetry visible without disrupting minimal aesthetics.",
      items: ["Runtime monitors threaded into hero", "Edge deployment slots enumerated", "Escalation ladder surfaced in-line"],
    },
  }), []);
  ...
  const protocols = [
    { name: "Signal intake", detail: "Audit the surface, define pulse cadence, ingest context in 24h.", status: "Ready" },
    { name: "Orbit sync", detail: "Cross-discipline huddle, assign vectors, prime release toggles.", status: "Armed" },
    { name: "Launch window", detail: "Go/no-go checks, activate uplink, monitor stability in realtime.", status: "Live" },
  ];
        ...
        <header className="grid gap-10 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.9fr)] lg:items-end">
          <div className="space-y-8">
            <div className="flex flex-wrap items-center gap-4">
              <span className={`inline-flex items-center gap-2 rounded-full border px-4 py-1 text-[10px] font-semibold uppercase tracking-[0.4em] ${palette.border} ${palette.accent}`}>Monochrome Hero Deck</span>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className={`inline-flex flex-wrap gap-3 rounded-full border px-5 py-3 text-xs uppercase tracking-[0.3em] transition ${palette.border} ${palette.accent}`}>
                <span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-current animate-pulse" />Launch ready</span>
          ...
          <div className={`relative flex flex-col gap-6 rounded-3xl border p-8 transition ${palette.border} ${palette.card}`}>
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => setMode("strategy")}
              ...
              {protocols.map((protocol, index) => (
                <li key={protocol.name} onMouseMove={setSpotlight} onMouseLeave={clearSpotlight}
                  ...
                  <div className="pointer-events-none absolute inset-0 opacity-0 transition duration-500 group-hover:opacity-100"
                    style={{ background: theme === "dark" ? "radial-gradient(190px circle at var(--hero3-x, 50%) var(--hero3-y, 50%), rgba(255,255,255,0.18), transparent 72%)" : "radial-gradient(190px circle at var(--hero3-x, 50%) var(--hero3-y, 50%), rgba(17,17,17,0.12), transparent 72%)" }} />
                  <div className="flex items-center justify-between">
